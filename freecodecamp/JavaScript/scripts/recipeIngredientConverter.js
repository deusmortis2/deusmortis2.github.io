...
const updateResultsList = () => {
  ...
  units.forEach((unit) => {
    if (unit !== unitToConvert.value) {
      ...
      const converted = processIngredient(Number(ingredientQuantity.value), unitToConvert.value, unit, numberOfServings.value);
      newList.textContent = `${ingredientName.value}: ${converted} ${unit}`;
